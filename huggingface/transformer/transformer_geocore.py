...
"""GeoCore Flow Matching DiT transformer (self-contained Diffusers model).

Architecture is adapted from the FLUX.2 reference implementation by Black Forest Labs
(https://github.com/black-forest-labs/flux), licensed under Apache-2.0, with GeoCore
geospatial metadata conditioning and the GSA projection head.
"""
...
class GeoCoreTransformer2DModel(ModelMixin, ConfigMixin):
    r"""GeoCore text- and geo-conditioned Flow Matching DiT.

    Self-contained FLUX.2-style double/single-stream transformer with geospatial
    metadata conditioning (`res`, `lon`, `lat`) and an optional GSA projection head.
    """
    ...
    def __init__(
        ...
        **_unused: Any,
        ...
        super().__init__()
        axes_dim = list(axes_dim) if axes_dim is not None else [32, 48, 48]

        self.gradient_checkpointing = False
        self.in_channels = in_channels
        self.out_channels = in_channels
        self.depth_repa = depth_repa
        ...
        self.y_in = nn.Linear(y_in_dim, self.hidden_size, bias=False)
